#!/usr/bin/env python3
"""Lightweight web UI for the Raspberry Pi radio.

Runs alongside radio.py — both talk to MPD via mpc.
No external dependencies beyond Python's standard library + PyYAML.

Usage:
    python3 web.py                 # default port 8080
    python3 web.py --port 9090     # custom port
"""
...
def mpc(*args):
    """Run an mpc command, return stdout."""
    try:
        r = subprocess.run(
            ["mpc"] + list(args),
...
def get_status():
    ...
    for line in lines:
        if line.startswith("[playing]"):
            ...
            status["state"] = "paused"
        if "volume:" in line:
            # e.g. "volume: 72%   repeat: off   random: off ..."
...
def play_station(bank_id, station_id):
    """Play a station by bank/station ID. Returns station name or None."""
    data = load_stations()
    banks = data.get("banks", {})
    ...
    if stype == "stream":
        ...
        if path:
            ...
            if not path.startswith("/"):
                # path is relative to audio root — MPD already knows it
                mpc("add", path)
            else:
                mpc("add", path)
            mpc("play")
        ...
        path = (station.get("path") or station.get("dir") or station.get("directory", "")).strip()
        if path:
            mpc("clear")
            mpc("repeat", "off")
            mpc("single", "off")
            mpc("random", "off")
            # Use mpc ls + add for directory contents
            if not path.startswith("/"):
                mpc("add", path)
            else:
                mpc("add", path)
...
(function(){
  ...
  let volTimer = null;
  let activeStation = null;   // "bankId-stationId"
  ...
  function playStation(bankId, stationId, btn){
    ...
    activeStation = bankId+"-"+stationId;
...
class RadioHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        if self.path == "/api/play":
            ...
            station_id = body.get("station")
            if bank_id is None or station_id is None:
                self._json({"error": "bank and station required"}, 400)
                return
            name = play_station(int(bank_id), int(station_id))
            if name:
            ...
            vol = body.get("volume")
            if vol is None:
                self._json({"error": "volume required"}, 400)
                return
...
def main():
    parser = argparse.ArgumentParser(description="Radio web UI")
    ...
    server = ThreadedHTTPServer((args.bind, args.port), RadioHandler)
    log.info("Radio web UI listening on http://%s:%d", args.bind, args.port)
    try:
